...
export const SchemaBuilder: React.FC<SchemaBuilderProps> = ({ doctors, services, fullGraph }) => {
  ...
  useEffect(() => {
    ...
    const city = doctors.length > 0 && doctors[0].city === 'chel' ? 'chelyabinsk' : 'spb';

    // Fetch analysis data from our new endpoint
    fetch(`/api/export/schema/analyze?city=${city}`)
      .then(res => res.json())
      .then(data => {
        if (data.success) {
  ...
  const generateDump = async () => {
    ...
    const city = doctors.length > 0 && doctors[0].city === 'chel' ? 'chelyabinsk' : 'spb';
    try {
      const res = await fetch(`/api/export/developer-dump?city=${city}`, {
        method: 'POST',
  ...
  const getEnrichedFields = (templateIds: number[], entities: any[], entityType?: string) => {
    const city = doctors.length > 0 && doctors[0].city === 'chel' ? 'chelyabinsk' : 'spb';

    if (city === 'chelyabinsk' && entities.length > 0) {
      // For Chelyabinsk, we just extract fields from the first entity
      const sampleEntity = entities[0];
      return Object.keys(sampleEntity).map(key => {
        let sample = "";
        for (const e of entities) {
          if (e[key] !== undefined && e[key] !== null && e[key] !== "") {
            sample = typeof e[key] === 'object' ? JSON.stringify(e[key]) : String(e[key]);
            break;
          }
        }
        return {
          field: key,
          caption: key,
          description: `Поле ${key}`,
          fillRate: 100,
          sample,
          type: typeof sampleEntity[key] === 'object' ? 'JSON' : 'String',
          isRelation: key.includes('id') && key !== 'id',
          relationTarget: key.includes('id') && key !== 'id' ? 'Связанная сущность' : ''
        };
      });
    }
  ...
  const doctorFields = getEnrichedFields([7], doctors, 'doctor');
  const serviceFields = getEnrichedFields([6, 32], services, 'service');
  const newsFields = getEnrichedFields([4], news, 'news');
  const promoFields = getEnrichedFields([19], promotions, 'promotion');
  const blogFields = getEnrichedFields([25], blog, 'blog');
  const pageFields = getEnrichedFields([2], pages, 'page');
  const reviewFields = getEnrichedFields([7], reviews, 'review');
  const programFields = getEnrichedFields([12], programs, 'program');
  const branchFields = getEnrichedFields([17], branches, 'branch');
  const equipmentFields = getEnrichedFields([8, 20, 29], equipment, 'equipment');
  const vacancyFields = getEnrichedFields([3], vacancies, 'vacancy');
            ...
            <>
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="p-6 border-b border-slate-200 bg-white">
                  <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2 mb-1">Схема: Врачи</h3>
                </div>
                {renderFieldTable(doctorFields, 'doctor')}
              </div>
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="p-6 border-b border-slate-200 bg-white">
                  <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2 mb-1">Схема: Услуги</h3>
                </div>
                {renderFieldTable(serviceFields, 'service')}
              </div>
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="p-6 border-b border-slate-200 bg-white">
                  <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2 mb-1">Схема: Отзывы</h3>
                </div>
                {renderFieldTable(reviewFields, 'review')}
              </div>
            </>
          ) : (
            <>
