...
export const ChelNewsList = ({ news, setCurrentDetail, setViewMode }: any) => {
  if (!news || news.length === 0) {
    return <div className="text-slate-500">Нет новостей для отображения.</div>;
  }
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {news.map((item: any) => (
        <div 
          key={item.id} 
          onClick={() => { setCurrentDetail({ type: 'Новость', data: item }); setViewMode('detail'); }} 
        ...
        >
          ...
          <div className="text-sm text-slate-500 line-clamp-3 mb-4" dangerouslySetInnerHTML={{ __html: item.anonce || item.content || 'Нет описания' }} />
