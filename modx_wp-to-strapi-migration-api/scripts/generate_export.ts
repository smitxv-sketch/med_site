...
async function generate() {
  try {
    ...
    console.log(`Exporting enhanced data to ${exportDir}${limit ? ` (Limit: ${limit})` : ''}...`);
    ...
    const dump: any = {
      _meta: {
        description: "Полный дамп базы данных MODX (СПб)",
    ...
    try {
      ...
      dump.translations = { content: locContent, tvs: locTVs };
    } catch (e) {}
    ...
    try {
      ...
      dump.settings = { clientConfig, sysSettings };
    } catch (e) {}
    ...
    try {
      ...
      dump.code = { chunks, snippets, plugins };
    } catch (e) {}
